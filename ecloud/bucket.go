...
func (p *Client) Batch(ctx Context, ret interface{}, op []string) (err error) {
	...
	return p.CallWithForm(ctx, ret, "POST", p.MgHost+"/batch", map[string][]string{"op": op})
}
...
func (p Bucket) Stat(ctx Context, key string) (entry Entry, err error) {
	err = p.Conn.Call(ctx, &entry, "POST", p.Conn.MgHost+URIStat(p.Name, key))
	return
}
...
func (p Bucket) Delete(ctx Context, key string) (err error) {
	return p.Conn.Call(ctx, nil, "POST", p.Conn.MgHost+URIDelete(p.Name, key))
}
...
func (p Bucket) Move(ctx Context, keySrc, keyDest string) (err error) {
	return p.Conn.Call(ctx, nil, "POST", p.Conn.MgHost+URIMove(p.Name, keySrc, p.Name, keyDest))
}
...
func (p Bucket) MoveEx(ctx Context, keySrc, bucketDest, keyDest string) (err error) {
	return p.Conn.Call(ctx, nil, "POST", p.Conn.MgHost+URIMove(p.Name, keySrc, bucketDest, keyDest))
}
...
func (p Bucket) Copy(ctx Context, keySrc, keyDest string) (err error) {
	return p.Conn.Call(ctx, nil, "POST", p.Conn.MgHost+URICopy(p.Name, keySrc, p.Name, keyDest))
}
...
func (p Bucket) ChangeMime(ctx Context, key, mime string) (err error) {
	return p.Conn.Call(ctx, nil, "POST", p.Conn.MgHost+URIChangeMime(p.Name, key, mime))
}
...
func (p Bucket) Fetch(ctx Context, key string, url string) (err error) {
	return p.Conn.Call(ctx, nil, "POST", p.Conn.MgHost+uriFetch(p.Name, key, url))
}
...
func (p Bucket) DeleteAfterDays(ctx Context, key string, days int) (err error) {
	return p.Conn.Call(ctx, nil, "POST", p.Conn.MgHost+URIDeleteAfterDays(p.Name, key, days))
}
...
func (p Bucket) makeListURL(prefix, delimiter, marker string, limit int) string {
	...
	return p.Conn.MgHost + "/list?" + query.Encode()
}
