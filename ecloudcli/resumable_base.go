...
func (p Uploader) resumableBput(
	...
	if ret.Ctx == "" {
		...
		if err != nil {
			log.Error("mkblk", blkIdx, "failed:", err)
			return
		}
		//log.Error("local crc32 =", ret.Crc32, "efs=", h.Sum32(), "efs offset", ret.Offset)
	...
	lzRetry:
		...
		if err == nil {
			//log.Error("loc.crc=", h.Sum32(), "efs crc=", ret.Crc32)
...
func (p Uploader) mkfile(
	...
	return p.Conn.CallWith(
		ctx, ret, "POST", url, "text/plain", bytes.NewReader(buf), len(buf))
}
